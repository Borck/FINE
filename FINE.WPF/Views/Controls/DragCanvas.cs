...
public class DragCanvas : Canvas {
  ...
  protected override void OnMouseMove(MouseEventArgs e) {
    ...
    if (_dragActive) {
      var curMouseScreenPos = e.GetPosition(this);

      if (!curMouseScreenPos.Equals(_previousMouseScreenPos)) {
        var xDelta = curMouseScreenPos.X - _previousMouseScreenPos.X;
        var yDelta = curMouseScreenPos.Y - _previousMouseScreenPos.Y;

        var dragEvent = new DragMoveEventArgs(e, xDelta, yDelta);
        DragMove?.Invoke(this, dragEvent);

        DragOffset = new Point(_previousDragOffset.X + xDelta, _previousDragOffset.Y + yDelta);
  ...
  protected override void OnMouseUp(MouseButtonEventArgs e) {
    ...
    if (_dragActive) {
      ...
      var curMouseScreenPos = e.GetPosition(this);
      var xDelta = curMouseScreenPos.X - _originScreenCoordPosition.X;
      var yDelta = curMouseScreenPos.Y - _originScreenCoordPosition.Y;
  ...
  private void ApplyDragToChildren(double deltaX, double deltaY) {
    foreach (UIElement cur in Children) {
      var prevLeft = GetLeft(cur);
      if (double.IsNaN(prevLeft)) {
        prevLeft = 0;
      }

      var prevTop = GetTop(cur);
      if (double.IsNaN(prevTop)) {
        prevTop = 0;
      }

      SetLeft(cur, prevLeft + deltaX);
      SetTop(cur, prevTop + deltaY);
    }
  ...
  public double MaxZoomFactor {
    get => (double)GetValue(MaxZoomFactorProperty); set => SetValue(MaxZoomFactorProperty, value);
  }
  ...
  public double MinZoomFactor {
    get => (double)GetValue(MinZoomFactorProperty); set => SetValue(MinZoomFactorProperty, value);
  }
  ...
  {
    var zoomModifier = curZoom / newZoom;
    var newSize = new Size(curView.Width * zoomModifier, curView.Height * zoomModifier);

    var zoomCenter = new Point(curView.X + curView.Width * relZoomPoint.X, curView.Y + curView.Height * relZoomPoint.Y);
    var newX = zoomCenter.X - relZoomPoint.X * newSize.Width;
    var newY = zoomCenter.Y - relZoomPoint.Y * newSize.Height;
  ...
  protected override void OnMouseWheel(MouseWheelEventArgs e) {
    ...
    var newScale = Math.Log(1 + wheelOffset / 10d) * 2d;
  ...
  private void SetZoomImpl(Point zoomCenter, double oldScale, double newScale, MouseEventArgs parentEvent) {
    ...
    var relZoomPoint = new Point {
      X = zoomCenter.X / ActualWidth,
      Y = zoomCenter.Y / ActualHeight
    };

    //Calculate new viewing window
    var newView = ZoomView(curView, oldScale, newScale, relZoomPoint);

    //Calculate new content offset based on the new view
    var newOffset = new Point(-newView.X * newScale, -newView.Y * newScale);
  ...
  public void SetViewport(Rect viewport) {
    ...
    DragOffset = new Point(-viewOffset.X * ZoomFactor, -viewOffset.Y * ZoomFactor);
  }
  #endregion
}

public class DragMoveEventArgs(MouseEventArgs mouseEvent, double deltaX, double deltaY) : EventArgs {
  public MouseEventArgs MouseEvent { get; } = mouseEvent;
  public double DeltaX { get; } = deltaX;
  public double DeltaY { get; } = deltaY;
}

public class ZoomEventArgs(MouseEventArgs e, ScaleTransform oldScale, ScaleTransform newScale, Point contentOffset) : EventArgs {
  public MouseEventArgs MouseEvent { get; } = e;
  public ScaleTransform OldScaleScale { get; } = oldScale;
  public ScaleTransform NewScale { get; } = newScale;
  public Point ContentOffset { get; } = contentOffset;
}
