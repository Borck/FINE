...
public class LongestCommonSubsequence {
  ...
  public static IEnumerable<(int index, T item, ChangeType change)> GetChanges<T>(IList<T> oldList, IList<T> newList) {
    ...
    var lcs = Calculate(oldList, newList).ToArray();

    //Initial data => LCS
    var lcsCursor = lcs.Length - 1;
    for (var initialDataCursor = oldList.Count - 1; initialDataCursor >= 0; initialDataCursor--) {
    ...
    //LCS => newdata
    lcsCursor = 0;
    for (var newDataCursor = 0; newDataCursor < newList.Count; newDataCursor++) {
