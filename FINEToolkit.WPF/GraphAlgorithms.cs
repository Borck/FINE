...
public class GraphAlgorithms {
  ...
  public static IEnumerable<ConnectionViewModel> FindLoops(NetworkViewModel network) {
    var nodesToCheck = new Stack<NodeViewModel>(network.Nodes.Items);
    var nodeStates = new Dictionary<NodeViewModel, NodeState>(nodesToCheck.Count);

    while (nodesToCheck.Count > 0) {
      var currentNode = nodesToCheck.Peek();
      ...
      var recursiveConnection = FindLoops(nodeStates, currentNode);
  ...
  private static ConnectionViewModel FindLoops(Dictionary<NodeViewModel, NodeState> nodeStates, NodeViewModel node) {
    ...
    var nodesToCheck = new List<ConnectionViewModel>();
    foreach (var input in node.Inputs.Items) {
      foreach (var con in input.Connections.Items) {
        var connectedNode = con.Output.Parent;
        ...
        if (connectedNodeState == NodeState.Ready) {
          ...
          //Found recursion!
          var keys = new List<NodeViewModel>(nodeStates.Keys);
    ...
    foreach (var con in nodesToCheck) {
      var currentNode = con.Output.Parent;
      var result = FindLoops(nodeStates, currentNode);
      if (result != null) {
        return result;
      }
    }
  ...
  public static IEnumerable<NodeViewModel> GetConnectedNodesTunneling(NodeViewModel startingNode, bool includeInputs = true, bool includeOutputs = false, bool includeSelf = false) {
    ...
    if (includeInputs) {
      var inputNodes = startingNode.Inputs.Items.SelectMany(i => i.Connections.Items).Select(c => c.Output.Parent);
      foreach (var nodeVM in inputNodes) {
        foreach (var subNodeVM in GetConnectedNodesTunneling(nodeVM, includeInputs, includeOutputs, true)) {
    ...
    if (includeOutputs) {
      var outputNodes = startingNode.Outputs.Items.SelectMany(i => i.Connections.Items).Select(c => c.Input.Parent);
      foreach (var nodeVM in outputNodes) {
        foreach (var subNodeVM in GetConnectedNodesTunneling(nodeVM, includeInputs, includeOutputs, true)) {
          if (subNodeVM != startingNode) {
            yield return subNodeVM;
          }
  ...
  /// A call to FindLoops is recommended before using this function
  /// </summary>
  public static IEnumerable<NodeViewModel> GetConnectedNodesBubbling(NodeViewModel startingNode, bool includeInputs = true, bool includeOutputs = false, bool includeSelf = false) => GetConnectedNodesTunneling(startingNode, includeInputs, includeOutputs, includeSelf).Reverse();
  ...
  public static IEnumerable<NodeViewModel> FindStartingNodes(NetworkViewModel network) => FindStartingNodes(network.Nodes.Items);
  ...
  public static IEnumerable<NodeViewModel> FindStartingNodes(IEnumerable<NodeViewModel> nodeGroup) {
    var todo = new Queue<NodeViewModel>(nodeGroup);
    var nodes = new HashSet<NodeViewModel>(todo);

    while (todo.Count > 0) {
      var cur = todo.Dequeue();

      var hasInputConnection = false;
      foreach (var input in cur.Inputs.Items) {
  ...
  public static IEnumerable<NodeViewModel> FindConnectedNodes(NodeViewModel sourceNode, IEnumerable<NodeViewModel> nodes) {
    var nodesSet = new HashSet<NodeViewModel>(nodes);
    ...
    while (nodeQueue.Count > 0) {
      ...
      foreach (var input in node.Inputs.Items) {
        foreach (var connectedNode in input.Connections.Items.Select(c => c.Output.Parent)) {
      ...
      foreach (var output in node.Outputs.Items) {
        foreach (var connectedNode in output.Connections.Items.Select(c => c.Input.Parent)) {
          if (visitedNodes.Add(connectedNode)) {
            nodeQueue.Enqueue(connectedNode);
          }
        }
  ...
  public static IEnumerable<IEnumerable<NodeViewModel>> FindSubGraphs(IEnumerable<NodeViewModel> nodes) {
    ...
    while (nodesSet.Count > 0) {
      var curNode = nodesSet.First();

      var subGraphMembers = new List<NodeViewModel>(FindConnectedNodes(curNode, nodesSet));
      foreach (var subGraphMember in subGraphMembers) {
  ...
  /// </summary>
  public static bool IsContinuousSubGraphSet(HashSet<NodeViewModel> nodesInSubGraphSet) => FindSubGraphs(FindStartingNodes(nodesInSubGraphSet))
  ...
  private static bool IsContinuousSubGroup(HashSet<NodeViewModel> groupNodesSet, IEnumerable<NodeViewModel> subGraphStartingNodes) {
    var queue = new Queue<NodeViewModel>(subGraphStartingNodes);
    var visitedNodes = new HashSet<NodeViewModel>(queue);
    ...
    while (queue.Count > 0) {
      ...
      foreach (var output in cur.Outputs.Items) {
        foreach (var con in output.Connections.Items) {
          var connectedNode = con.Input.Parent;
