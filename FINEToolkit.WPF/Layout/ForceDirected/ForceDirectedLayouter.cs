namespace FINE.Toolkit.Layout.ForceDirected;
...
public class ForceDirectedLayouter {
  ...
  public void Layout(Configuration config, int maxIterations) {
    ...
    var deltaT = (int)Math.Ceiling(10.0 / (double)config.UpdatesPerIteration);
    for (var i = 0; i < maxIterations * config.UpdatesPerIteration; i++) {
  ...
  public async Task LayoutAsync(Configuration config, CancellationToken token) {
    ...
    do {
      ...
      var virtDeltaT = (int)(deltaT.Milliseconds * config.TimeModifier);
      var virtDeltaTPerUpdate = virtDeltaT / config.UpdatesPerIteration;
      for (var i = 0; i < config.UpdatesPerIteration; i++) {
