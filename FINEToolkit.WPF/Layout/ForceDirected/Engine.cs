...
internal class Engine {
  ...
  internal void Update(int deltaTMillis, IState state, Configuration config) {
    // Calculate forces
    var nodeCount = config.Network.Nodes.Count;
    ...
    foreach (var (node, force) in nodeForces) {
      var speed = state.GetNodeSpeed(node);
      var pos = state.GetNodePosition(node);
      var deltaT = deltaTMillis / 1000.0;
