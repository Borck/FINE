...
public class ValueNodeGroupIOBinding : NodeGroupIOBinding {
  ...
  public virtual ValueNodeOutputViewModel<T> CreateCompatibleOutput<T>(ValueNodeInputViewModel<T> input) => new ValueNodeOutputViewModel<T>() {
    Name = input.Name,
    Icon = input.Icon
  };

  public virtual ValueNodeOutputViewModel<IObservableList<T>> CreateCompatibleOutput<T>(ValueListNodeInputViewModel<T> input) => new ValueNodeOutputViewModel<IObservableList<T>>();

  public virtual ValueNodeInputViewModel<T> CreateCompatibleInput<T>(ValueNodeOutputViewModel<T> output) => new ValueNodeInputViewModel<T>() {
    Name = output.Name,
    Icon = output.Icon
  };

  public virtual ValueListNodeInputViewModel<T> CreateCompatibleInput<T>(ValueNodeOutputViewModel<IObservableList<T>> output) => new ValueListNodeInputViewModel<T>() {
    Name = output.Name,
    Icon = output.Icon
  };
  #endregion
  ...
  public override NodeInputViewModel GetGroupNodeInput(NodeOutputViewModel entranceOutput) => _outputInputMapping[entranceOutput];

  public override NodeOutputViewModel GetSubnetInlet(NodeInputViewModel entranceInput) => _outputInputMapping.Single(p => p.Value == entranceInput).Key;

  public override NodeInputViewModel GetSubnetOutlet(NodeOutputViewModel exitOutput) => _outputInputMapping[exitOutput];

  public override NodeOutputViewModel GetGroupNodeOutput(NodeInputViewModel exitInput) => _outputInputMapping.Single(p => p.Value == exitInput).Key;
  #endregion
  ...
  public virtual void DeleteEndpoint(Endpoint endpoint) {
    ...
    var isOnGroupNode = GroupNode == endpoint.Parent;
