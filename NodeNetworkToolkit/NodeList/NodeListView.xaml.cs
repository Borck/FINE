...
public partial class NodeListView : IViewFor<NodeListViewModel> {
  ...
  public bool ShowDisplayModeSelector { get => (bool)GetValue(ShowDisplayModeSelectorProperty); set => SetValue(ShowDisplayModeSelectorProperty, value);
  }
  ...
  public Brush ListEntryBackgroundBrush { get => (Brush)GetValue(ListEntryBackgroundBrushProperty); set => SetValue(ListEntryBackgroundBrushProperty, value);
  }
  ...
  public Brush ListEntryBackgroundMouseOverBrush { get => (Brush)GetValue(ListEntryBackgroundMouseOverBrushProperty); set => SetValue(ListEntryBackgroundMouseOverBrushProperty, value);
  }
  ...
  public Brush ListEntryHandleBrush { get => (Brush)GetValue(ListEntryHandleBrushProperty); set => SetValue(ListEntryHandleBrushProperty, value);
  }
  #endregion
  ...
  private void OnNodeMouseMove(object sender, MouseEventArgs e) {
    if (e.LeftButton == MouseButtonState.Pressed) {
      var nodeVM = ((FrameworkElement)sender).DataContext as NodeViewModel;
      if (nodeVM == null) {
        return;
      }

      var nodeFactory = ViewModel.NodeTemplates.Items.First(t => t.Instance == nodeVM).Factory;
      var newNodeVM = nodeFactory();
