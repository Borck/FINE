...
public partial class ShaderPreviewView : IViewFor<ShaderPreviewViewModel> {
  ...
  public Shader VertexShader {
    get => (Shader)GetValue(VertexShaderProperty);
    set {
      VertexShader?.Dispose();
      SetValue(VertexShaderProperty, value);
    }
  }
  ...
  public Shader FragmentShader {
    get => (Shader)GetValue(FragmentShaderProperty);
    set {
      FragmentShader?.Dispose();
      SetValue(FragmentShaderProperty, value);
    }
  }
  ...
  public ShaderProgram ShaderProgram {
    get => (ShaderProgram)GetValue(ShaderProgramProperty);
    set {
      ShaderProgram?.Dispose();
      SetValue(ShaderProgramProperty, value);
    }
  }
  ...
  private void LoadMeshes() {
    using (var stream = new StreamReader(typeof(ShaderPreviewView).Assembly.GetManifestResourceStream("FINE.Resources.Models.cube.obj"))) {
      _cube = WavefrontLoader.Load(stream);
    }

    using (var stream = new StreamReader(typeof(ShaderPreviewView).Assembly.GetManifestResourceStream("FINE.Resources.Models.suzanne.obj"))) {
  ...
  */
  ...
  private void GlControl_Render(object sender, EventArgs e) {
    if (ViewModel != null) {
      ...
      var seconds = (float)(DateTime.Now - startTime).TotalMilliseconds / 1000f;
