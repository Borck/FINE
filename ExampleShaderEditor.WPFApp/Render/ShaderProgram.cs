...
public class ShaderProgram : IDisposable {
  ...
  public bool SetUniformFloat(string argumentName, float value) {
    GL.UseProgram(Id);
    var varLoc = GL.GetUniformLocation(Id, argumentName);
    if (varLoc == -1) {
      return false;
    }
  ...
  public bool SetUniformMatrix(string argumentName, Matrix<float> values, bool transpose = false) {
    var array = values.AsColumnMajorArray() ?? values.ToColumnMajorArray();
  ...
  public bool SetUniformMatrix(string argumentName, Matrix<double> values, bool transpose = false) {
    var array = (values.AsColumnMajorArray() ?? values.ToColumnMajorArray()).Select(c => (float)c).ToArray();
    return SetUniformMatrix(argumentName, array, values.RowCount, values.ColumnCount, transpose);
  }

  private float[] MatrixToFloatArray(Matrix3 m) => new[]
    {
              m.M11, m.M21, m.M31,
              m.M12, m.M22, m.M32,
              m.M13, m.M23, m.M33,
  ...
  private float[] MatrixToFloatArray(Matrix4 m) => new[]
    {
