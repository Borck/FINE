...
public class Renderer {
  ...
  private void RenderChildren(RenderObject obj, Matrix4 viewProjectionMatrix, Camera camera, float time) {
    ...
    if (obj.Model?.Mesh != null && obj.Model?.Shader != null) {
      var cameraPos = camera.GetWorldPosition().Subtract(obj.GetWorldPosition());
  ...
  private Matrix4 CreateViewMatrix(Camera camera) => MathMatrixToOpenglMatrix(camera.BuildViewMatrix());
  ...
  private Matrix<double> OpenGlMatrixToMathMatrix(Matrix4 m) => Matrix<double>.Build.DenseOfRowArrays(
